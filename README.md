...
- **Gérer une playlist** : le bouton « … » qui apparaît au survol d’une playlist dans la barre latérale (ou un clic droit) propose **Renommer**, **Exporter en M3U**, **Actualiser** et **Supprimer**. Le crayon à côté du titre et le double-clic renomment aussi.
- **Exporter en M3U** : une, plusieurs ou toutes les playlists (**Sources & guide TV → Exporter en M3U…**), dans un seul fichier (adresses en double retirées) ou un fichier par playlist. Groupes, logos, `tvg-id`, guide TV (`x-tvg-url`) et en-têtes HTTP (`#EXTVLCOPT`) sont conservés, et le fichier se relit dans Fluxo, VLC, Kodi ou TiviMate. Les adresses Xtream exportées contiennent l’identifiant et le mot de passe du compte ; les séries, qui n’ont pas d’adresse propre, ne sont pas exportées.
...
- Glissez un ou plusieurs fichiers audio/vidéo, ou des dossiers, dans la fenêtre : trois zones apparaissent. **Lire maintenant** les ajoute à **Médias locaux** et lance le premier fichier ; **Ajouter à la suite** les place après le média en cours sans l’interrompre ; **Convertir** les envoie au convertisseur. La liste **À suivre** permet de passer au précédent ou au suivant ; la lecture continue automatiquement au fichier suivant. Le bouton **Ouvrir des médias** accepte aussi plusieurs fichiers.
- **Débit** : pendant la lecture, le débit du flux s’affiche sous la vidéo et dans la barre du lecteur (moyenne sur quelques secondes, mesurée par mpv, par le relais local ou par le lecteur de macOS). Son infobulle et ⓘ détaillent la vidéo, l’audio et les données reçues.
- **Plan de la vidéo** : pour un film, un épisode ou un fichier, le bouton pellicule (ou `P`) affiche une bande d’images de toute la vidéo, comme QuickTime ; cliquez ou glissez dessus pour vous déplacer, `OK` ou `Échap` la ferme. Une fois les images prises, survoler la barre de lecture en montre un aperçu. Pour les vidéos en ligne, les images sont prises avec une seule connexion supplémentaire.
- **Stop** (■ ou `S`) arrête la lecture et libère la connexion au fournisseur ; la file « À suivre » est conservée. Un clic sur l’image met en pause.
- En plein écran et en mode lecteur, les commandes et le pointeur disparaissent après 3 secondes de lecture sans mouvement.
- **Convertisseur** (barre latérale) : convertit vidéos et fichiers audio en MP4 H.264 ou HEVC, MKV, WebM VP9, MP4 AV1, MOV ProRes, AVI, GIF, ou extrait le son (MP3, M4A, FLAC, WAV). **Sans réencodage** change seulement de conteneur, en quelques secondes et sans perte. Définition, qualité, encodage matériel VideoToolbox (avec repli logiciel automatique) et dossier de destination se règlent ; les conversions s’enchaînent en arrière-plan avec progression, vitesse et temps restant, et le fichier d’origine n’est jamais modifié. Les enregistrements se convertissent aussi depuis **Enregistrements**.
- Passez en **Mode lecteur** pour agrandir l’image, ou utilisez le bouton plein écran. Les commandes incluent lecture/pause, saut de 10 secondes, position, volume, vitesse et image dans l’image quand macOS l’autorise. Les raccourcis `Espace`, `←`, `→`, `M`, `F`, `S` (stop) et `P` (plan) agissent lorsque le focus n’est pas dans un champ ; `↑`/`↓` changent de chaîne, les chiffres donnent accès à un numéro et `⌫` revient à la chaîne précédente.
...
- `src-tauri` : intégration macOS, API Xtream, trousseau, sonde de flux, relais local, segmenteur TS, FFmpeg, moteur mpv (`mpv/` : chargement de libmpv et rendu macOS ; `video_engine.rs` : surfaces et commandes), mises à jour, export M3U (`export.rs`), convertisseur (`converter.rs`), vignettes (`thumbnails.rs`), guide TV, vérifications de disponibilité, enregistrements et persistance (`state.json` pour l’état utilisateur, `catalog/` pour les chaînes de chaque playlist).
- `src` : interface ; `icons.js` (icônes [Lucide](https://lucide.dev)), `bitrate.js` (mesure du débit), `filmstrip.js` (plan de la vidéo), `playback.js` (choix du moteur, escalade et bascule), `mpv-media.js` (mpv vu comme un élément vidéo), `catalog.js` (doublons, filtres, sources alternatives), `guide-grid.js`, `multiview.js`.
